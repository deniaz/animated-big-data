...
var LayoutEngine=function(){"use strict";function t(t){return Math.pow(t,4)/30}function e(){this.attributes=[]}function r(){this.nodes=[],this.links=[]}function n(){return l}function u(){return d}function a(t,e){for(var r=0,n=0;n<t.attributes.length;n++)for(var u=0;u<e.attributes.length;u++)if(e.attributes[u].id==t.attributes[n].id){r++;break}return r}function i(n,u,i){for(var b=[],c=n.length,p=0;p<n.length-1;p++)for(var g=p+1;g<n.length;g++){var v={indexA:p,indexB:g,equal:a(n[p],n[g])};b.push(v)}b.sort(function(t,e){return t.equal<e.equal?1:t.equal>e.equal?-1:0});for(var y=[],q=[],p=0;p<b.length;p++){for(var k=b[p],x=n[k.indexA].frequency,G=n[k.indexB].frequency,m=!1,g=0;g<y.length;g++)if(y[g].frequency.label==x.label||y[g].frequency.label==G.label){m=!0;break}if(!m){var A={subGraphA:n[k.indexA],subGraphB:n[k.indexB]};q.push(A),y.push(A.subGraphA),y.push(A.subGraphB)}}if(c%2!=0)for(var p=0;p<n.length;p++){for(var B=!1,g=0;g<y.length;g++)if(n[p].frequency.label==y[g].frequency.label){B=!0;break}if(!B){var A={subGraphA:n[p],subGraphB:null};q.push(A),y.push(A.subGraphA)}}for(var L=[],N=new e,p=0;p<q.length;p++){for(var w=q[p].subGraphA.attributes,K=[],O=new e,g=0;g<w.length;g++)1==w[g].numberOfLinks?K.push(w[g]):O.add(w[g]);if(L.push(K),null!=q[p].subGraphB){w=q[p].subGraphB.attributes,K=[];for(var g=0;g<w.length;g++)1==w[g].numberOfLinks?K.push(w[g]):O.add(w[g]);L.push(K)}O.attributes.sort(function(t,e){return t.numberOfLinks>e.numberOfLinks?1:t.numberOfLinks<e.numberOfLinks?-1:0});for(var g=0;g<O.attributes.length;g++)N.add(O.attributes[g])}for(var E=t(y[0].frequency.intervals[0].percentage),p=0;p<y.length;p++)for(var g=0;g<y[p].frequency.intervals.length;g++){var F=t(y[p].frequency.intervals[g].percentage);F>E&&(E=F)}for(var M=u/2-s/2,j=M-h-E,z=j-E-h-s,C=M+s+h+E,D=C+E+h,H=0,I=0,J=new r,p=0;p<N.attributes.length;p++)N.attributes[p].x=M,N.attributes[p].y=I,I+=o+f,J.addNode(N.attributes[p]);var P=N.attributes.length;I=(P*o+(P-1)*f)/2;for(var p=0;p<y.length;p++){var Q=y[p].frequency,R=L[p].length;if(p%2==0){Q.x=j;for(var g=0;R>g;g++)L[p][g].x=z}else{Q.x=C;for(var g=0;R>g;g++)L[p][g].x=D}p>=2&&(I+=2*E+h),Q.y=I;for(var S=I-(R*o+(R-1)*f)/2,g=0;R>g;g++)L[p][g].y=S,J.addNode(L[p][g]),S+=o+f;J.addNode(Q)}var T=L[y.length-1][R-1].y+o,U=y[y.length-1].frequency.y+E;H=T>U?T:U;for(var V=(i-H)/2,p=0;p<J.nodes.length;p++)J.nodes[p].y+=V;for(var p=0;p<y.length;p++)for(var g=0;g<y[p].attributes.length;g++)J.link(y[p].frequency,J.nodes[J.getKey(y[p].attributes[g])]);l=J.nodes,d=J.links}var s=120,o=45,f=15,h=50;e.prototype.getKey=function(t){for(var e=0;e<this.attributes.length;e++){var r=this.attributes[e];if(r.id&&t.id&&r.id===t.id)return e}throw Error("Element not foung.")},e.prototype.contains=function(t){try{return this.getKey(t),!0}catch(e){return!1}},e.prototype.add=function(t){this.contains(t)||this.attributes.push(t)},r.prototype.getKey=function(t){for(var e=0;e<this.nodes.length;e++){var r=this.nodes[e];if(r.id&&t.id&&r.id===t.id)return e}throw Error("Element not found.")},r.prototype.containsNode=function(t){try{return this.getKey(t),!0}catch(e){return!1}},r.prototype.addNode=function(t){return this.containsNode(t)?this.nodes[this.getKey(t)]:this.nodes.push(t)-1},r.prototype.link=function(t,e){this.links.push({source:t,target:e})};var l=[],d=[];return{getNodes:n,getLinks:u,buildFromArray:i}}();
